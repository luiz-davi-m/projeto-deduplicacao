...
def classificar(caminho_csv, nome_saida):
    ...
    features = pares[["authors_sim", "title_sim", "venue_date_eq"]]
    features.index = pairs_index

    # Cria MultiIndex para os casos positivos
    match_index = pd.MultiIndex.from_arrays(
        [pares.loc[pares["id_1"] == pares["id_2"], "idx1"],
         pares.loc[pares["id_1"] == pares["id_2"], "idx2"]],
        names=["idx1", "idx2"]
    )
    ...
    pares_indexed["match_verdadeiro"] = (pares_indexed["id_1"] == pares_indexed["id_2"]).astype(int)
